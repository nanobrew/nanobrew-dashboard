<template>
  <v-container fluid>
    <v-row>
      <v-col cols="12" lg="8">
        <div v-if="!Object.keys(sensors).length">
          <h2>No sensors configured</h2>
          <p>Go to the equipment page to configure new sensors.</p>
        </div>
          ...
          <v-col v-for="(sensor, key) in sensors" :key="key" cols="12" sm="6" md="3" lg="3">
            <v-card class="ma-3 pa-4 nb-card" tile>
              ...
              <div class="nb-measurement py-4">
                <span class="nb-value">{{sensor.value}}</span>

                <span class="nb-unit">{{sensor.unit.symbol}}</span>
              </div>
            </v-card>
          </v-col>
        </v-row>
